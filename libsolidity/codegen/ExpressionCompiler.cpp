...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
        ...
        Instruction ds_storage_ins = Instruction::STOP;
		...
		case FunctionType::Kind::GasLeft:
			...
			break;
        case FunctionType::Kind::DS_READ_VERSION:
            {
			_functionCall.expression().accept(*this);
            ...
            m_context<<Instruction::DATAVERSIONREAD;
            break;
            }
        case FunctionType::Kind::DS_UPDATE:
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::DATAVERSIONUPDATE;
        case FunctionType::Kind::DS_SGAS:
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::STORAGE_GAS;
        case FunctionType::Kind::DS_SLICE:
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::STORAGE_GAS_PRICE;
        case FunctionType::Kind::DS_SUSED:
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::STORAGE_GAS_USED;
        case FunctionType::Kind::DS_SSIZE:
            {
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::SSIZE;
			_functionCall.expression().accept(*this);
			solAssert(arguments.size() == 1, "");
			// push the argumnet
			arguments[0]->accept(*this);
            //call the customed instruction
            m_context<<ds_storage_ins;
            //clean stack
            //m_context << swapInstruction(arguments.size()+1) << Instruction::POP;
            //m_context << Instruction::POP;
            break;
            }
        case FunctionType::Kind::DS_CHK_SENTINEL:
            {
            ds_storage_ins = ds_storage_ins != Instruction::STOP ? ds_storage_ins: Instruction::SENTINEL_HEFT;
			_functionCall.expression().accept(*this);
            //call the customed instruction
            m_context<<ds_storage_ins;
            //clean stack
...
bool ExpressionCompiler::visit(MemberAccess const& _memberAccess)
{
	...
	case Type::Category::Array:
	{
        ...
        else if (member == "read_version")
		{
			solAssert(!type.isDynamicallySized() && type.location() == DataLocation::Memory, 
                      "The array must be a non-dynamically memory array");
			solAssert(type.length() == 16, "The array length must be 16");
            if(IntegerType const* baseType = dynamic_cast<IntegerType const*>(type.baseType().get()))
			    solAssert(baseType->category() == Type::Category::Integer && !baseType->isSigned(), "The array base type must be unsigned int");
            else
			    solAssert(false, "The array base type is not int.");
		}
		else
	...
	case Type::Category::d_storage:
        //just break, d_storage is address
        break;
	default:
