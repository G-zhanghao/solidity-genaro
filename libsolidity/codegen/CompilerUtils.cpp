...
)
{
    ...
    case Type::Category::d_storage:
        {
		solAssert(targetTypeCategory == Type::Category::Integer || targetTypeCategory == Type::Category::d_storage, "d_storage can't convert to this type");
        if (targetTypeCategory == Type::Category::d_storage)
            break;
        d_storageType const& typeOnStack = dynamic_cast<d_storageType const&>(_typeOnStack);
        IntegerType const& targetType = dynamic_cast<IntegerType const&>(_targetType);
        if (targetType.numBits() < typeOnStack.numBits())
			    m_context << ((u256(1) << targetType.numBits()) - 1) << Instruction::AND;
        break;
        }
	...
	case Type::Category::RationalNumber:
		...
		else if (targetTypeCategory == Type::Category::d_storage)
        {
			if (stackTypeCategory == Type::Category::RationalNumber)
            {
				RationalNumberType const& typeOnStack = dynamic_cast<RationalNumberType const&>(_typeOnStack);
                if (typeOnStack.isFractional() || typeOnStack.isNegative())
                    solAssert(
				    false,
				    "Invalid conversion from negative or fractional number to " + _targetType.toString(false) + ".");

                //clean high bit
			    d_storageType const& targetRealType = dynamic_cast<d_storageType const&>(_targetType);
			    m_context << ((u256(1) << targetRealType.numBits()) - 1) << Instruction::AND;
            }
            else if (stackTypeCategory == Type::Category::Integer)
            {

				IntegerType const& typeOnStack = dynamic_cast<IntegerType const&>(_typeOnStack);
                if (!typeOnStack.isAddress())                
                    solAssert(
				    false,
				    "Invalid conversion from int to " + _targetType.toString(false) + ".");
            }
            else
                solAssert(
				false,
				"Invalid conversion from rational number to " + _targetType.toString(false) + ".");
            break;
        }    
		else
		{
