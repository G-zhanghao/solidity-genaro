...
TypePointer Type::fromElementaryTypeName(ElementaryTypeNameToken const& _type)
{
	...
	case Token::D_STORAGE:
		return make_shared<d_storageType>();
	//no types found
	default:
		solAssert(
			false,
...
bool IntegerType::isImplicitlyConvertibleTo(Type const& _convertTo) const
{
    ...
    if (_convertTo.category() == Category::d_storage)
    {
        if (!isSigned())
            return true;
        return false;
...
bool IntegerType::isExplicitlyConvertibleTo(Type const& _convertTo) const
{
	return _convertTo.category() == category() ||
        ...
        _convertTo.category() == Category::d_storage;
}
...
TypePointer IntegerType::binaryOperatorResult(Token::Value _operator, TypePointer const& _other) const
{
	if (
        ...
        _other->category() != Category::d_storage &&
...
d_storageType::d_storageType(): m_bits(160)
{
}

string d_storageType::richIdentifier() const
{
    return "d_storage";
}

//not permit to implicitly convert to other types
bool d_storageType::isImplicitlyConvertibleTo(Type const& _convertTo) const
{
...
bool d_storageType::isExplicitlyConvertibleTo(Type const& _convertTo) const
{
    if (_convertTo.category() == Category::Integer){
        IntegerType const* convertTo = dynamic_cast<IntegerType const*>(&_convertTo);
        if (convertTo && convertTo->isAddress())
            return true;
        if (convertTo && !convertTo->isSigned())
            return true;
    }
    return false;
}

bool d_storageType::operator==(Type const& _other) const
{
...
string d_storageType::toString(bool) const
{
    return "d_storage";
}


TypePointer d_storageType::binaryOperatorResult(Token::Value _operator, TypePointer const& _other) const
{
    if (_other->category() == category())
        return _other;
	if (_other->category() == Category::Integer){
        IntegerType const& convertTo = dynamic_cast<IntegerType const&>(*_other);
        if (!convertTo.isSigned())
	        if (Token::isCompareOp(_operator))
		        return _other;
    }
    if (_other->category() == Category::RationalNumber){
        RationalNumberType const& convertTo = dynamic_cast<RationalNumberType const&>(*_other);
        if (!convertTo.isNegative() && !convertTo.isFractional())
	        if (Token::isCompareOp(_operator))
		        return _other;
    }
...
TypePointers d_storageType::parseElementaryTypeVector(strings const& _types)
{
...
MemberList::MemberMap d_storageType::nativeMembers(ContractDefinition const*) const
{
    return {/*
			{"read_version", make_shared<FunctionType>(strings{"uint256","uint256"}, 
                                                       strings{"uint64","uint[] memory","uint[] memory"},
                                                       FunctionType::Kind::DS_READ_VERSION,
                                                       false, StateMutability::View)},*/
			{"update", make_shared<FunctionType>(strings{"uint256"}, strings{"bool"}, FunctionType::Kind::DS_UPDATE, false, StateMutability::View)},
			{"sgas", make_shared<FunctionType>(strings{"uint256"}, strings{"uint64"}, FunctionType::Kind::DS_SGAS, false, StateMutability::View)},
			{"slice", make_shared<FunctionType>(strings{"uint256"}, strings{"uint64"}, FunctionType::Kind::DS_SLICE, false, StateMutability::View)},
			{"sused", make_shared<FunctionType>(strings{"uint256"}, strings{"uint64"}, FunctionType::Kind::DS_SUSED, false, StateMutability::View)},
			{"ssize", make_shared<FunctionType>(strings{"uint256"}, strings{"uint64"}, FunctionType::Kind::DS_SSIZE, false, StateMutability::View)},
			{"check_sentinel", make_shared<FunctionType>(strings(), strings{"uint64"}, FunctionType::Kind::DS_CHK_SENTINEL, false, StateMutability::View)}
    };
}

bigint d_storageType::minValue() const
{
    return bigint(0);
}

bigint d_storageType::maxValue() const
{
    return (bigint(1) << m_bits) - 1;
}
...
bool RationalNumberType::isImplicitlyConvertibleTo(Type const& _convertTo) const
{
	if (_convertTo.category() == Category::d_storage)
    {
        ...
        if (isNegative())
            return false;
		d_storageType const& targetType = dynamic_cast<d_storageType const&>(_convertTo);
        if (m_value > targetType.maxValue())
            return false;
        return true;
    }
	if (_convertTo.category() == Category::Integer)
...
MemberList::MemberMap ArrayType::nativeMembers(ContractDefinition const*) const
{
	...
	if (!isString())
	{
        ...
        if (!isDynamicallySized()){
            members.push_back({"read_version", make_shared<FunctionType>(strings{"address","uint256","uint256"}, 
                                                       strings{"uint64"},
                                                       FunctionType::Kind::DS_READ_VERSION,
                                                       false, StateMutability::View
            )});
...
string FunctionType::richIdentifier() const
{
    ...
    case Kind::DS_READ_VERSION: id += "read_version"; break;
    case Kind::DS_UPDATE: id += "update"; break;
    case Kind::DS_SGAS: id += "sgas"; break;
    case Kind::DS_SLICE: id += "slice"; break;
    case Kind::DS_SUSED: id += "sused"; break;
    case Kind::DS_SSIZE: id += "ssize"; break;
	case Kind::DS_CHK_SENTINEL: id += "check_sentinel"; break;
