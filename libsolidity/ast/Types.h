 ...
 * d_storage for GNX
 */
class d_storageType: public Type
{
public:
	virtual Category category() const override { return Category::d_storage; }

	explicit d_storageType();
...
public:
	...
	enum class Kind
	{
		...
		GasLeft, ///< gasleft()
        DS_READ_VERSION, ///< read_version() for d_storage
        DS_UPDATE, ///< update() for d_storage
        DS_SGAS, ///< sgas() for d_storage
        DS_SLICE, ///< slice() for d_storage
        DS_SUSED, ///< sused() for d_storage
        DS_SSIZE, ///< ssize() for d_storage
        DS_CHK_SENTINEL ///< check_sentinel() for d_storage
	};
